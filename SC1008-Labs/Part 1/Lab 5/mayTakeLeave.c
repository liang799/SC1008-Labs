...
void printList(leaveRecord list[], int n) {
    ...
    for (p = 0; p < n; p++)
        printf("id = %d, totalleave = %d, leave taken = %d\n", list[p].id,
               list[p].totalLeave, list[p].leaveTaken);
}

// *n is to return the thingy
void getInput(leaveRecord list[], int *n) {
    int i;
    leaveRecord r;
    ...
    for (i = 0; i < *n; i++) {
        ...
        scanf("%d %d %d", &r.id, &r.totalLeave, &r.leaveTaken);
        list[i] = r;
    }
}
/*
    Approval will be
    given if the leave taken so far plus the number of days applied for is less
   than or equal to his total number of leave days allowed.
 */
int mayTakeLeave(leaveRecord list[], int id, int leave, int n) {
    int i;
    int leaveBalance = 0;
    ...
    for (i = 0; i < n; i++) {
        ...
        leaveBalance = list[i].totalLeave - list[i].leaveTaken;
